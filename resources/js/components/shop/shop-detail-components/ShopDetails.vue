        ...
        <div class="d-flex align-items-start justify-content-between flex-wrappp">
            ...
            <div class="right ms-4 flex-shrink-0 sec-col" v-if="role == 'Client'">
                <div class="select-parent position-relative">
                    <input type="date" v-model="booking_date" class="form-control select-date" readonly />
                </div>
                <div class="est-time fw-500 mt-1 text-orange small d-flex align-items-center justify-content-end">
                    <small class="custom-pad">Shop No <i class="fa fa-phone" aria-hidden="true"></i> </small>
                    <small class="fw-normal text-color-1 d-flex align-items-center">
                        <!--                         <svg
                    class="mx-2"
                    width="11"
                    height="11"
        ...
        <div class="categories mt-4 mb-5">
            ...
            <div class="category-tabs" v-if="props.data.user_categories.length">
                <MDBTabs v-model="activeTabId4" vertical>
                    <MDBTabNav tabsClasses="my-3">
                        <MDBTabItem class="fw-bold mb-3 py-2 px-0 text-capitalize d-flex align-items-end" v-for="(category, index) in props.data
                    .user_categories" :key="index" :wrap="false" :tabId="`category-${category.id}`" :href="`category-${category.id}`">{{ category.category.name }}
                        </MDBTabItem>
                    </MDBTabNav>
                    <MDBTabContent>
                        <MDBTabPane class="category-tab-pane" :tabId="`category-${category.id}`" v-for="(category, index) in categtoriesArray" :key="index">
                            <div v-if="
                        category.user_business_category_services
                        .length
                        ">
                                <div class="d-flex align-items-center justify-content-between border-bottom py-3" v-for="(
                            subcategory, index
                            ) in category.user_business_category_services" :key="index">
                                    <small><span v-if="role == 'Client'"><label :for="
                                'service-' +
                                ...
                                " />
                                    ...
                                    " class="ms-2" width="15" height="15" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M0.999787 11.0002H1.99979V18.0002C1.99979 19.1032 2.89679 20.0002 3.99979 20.0002H15.9998C17.1028 20.0002 17.9998 19.1032 17.9998 18.0002V11.0002H18.9998C19.1975 11.0002 19.3908 10.9415 19.5552 10.8316C19.7197 10.7217 19.8478 10.5656 19.9235 10.3829C19.9991 10.2002 20.0189 9.99912 19.9804 9.80517C19.9418 9.61122 19.8466 9.43305 19.7068 9.2932L10.7068 0.293201C10.614 0.200255 10.5038 0.126518 10.3825 0.0762068C10.2612 0.0258961 10.1311 0 9.99979 0C9.86845 0 9.73841 0.0258961 9.61709 0.0762068C9.49578 0.126518 9.38558 0.200255 9.29279 0.293201L0.292787 9.2932C0.152977 9.43305 0.057771 9.61122 0.0192035 9.80517C-0.0193641 9.99912 0.000438951 10.2002 0.076109 10.3829C0.151779 10.5656 0.279918 10.7217 0.444328 10.8316C0.608738 10.9415 0.802037 11.0002 0.999787 11.0002ZM7.99979 18.0002V13.0002H11.9998V18.0002H7.99979ZM9.99979 2.4142L15.9998 8.4142V13.0002L16.0008 18.0002H13.9998V13.0002C13.9998 11.8972 13.1028 11.0002 11.9998 11.0002H7.99979C6.89679 11.0002 5.99979 11.8972 5.99979 13.0002V18.0002H3.99979V8.4142L9.99979 2.4142Z" fill="#8C96A2" />
                                        </svg>
                                    </small>
                                    <small class="text-orange fw-bold price">{{ subcategory.charges + " Rs" }}
                                    </small>
                                </div>
                            </div>
            ...
            <div class="text-end mt-4" v-if="role == 'Client'">
               <!--  <router-link :to="{
                        ...
                        "> -->
                    <MDBBtn @click="requestHandle()" :disabled="selectedCategories.length === 0" class="text-white bg-orange fw-500 text-capitalize rounded-4 shadow-0 book-btn">
                        Check in
                    </MDBBtn>
                <!-- </router-link> -->
                <MDBRow class="align-items-center ">
                    <MDBCol col="12 col-sm-8 col-md-12 custom-margin">
                        <div class="reviews-parrent">
                            <div class="">
                                <ul class="nav nav-tabs" id="myTab" role="tablist">
                                    ...
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link" id="location-tab" data-bs-toggle="tab" data-bs-target="#reviews" type="button" role="tab" aria-controls="reviews" aria-selected="false">
                                            Reviews
                                        </button>
                                    </li>
                                </ul>
                                <div class="tab-content" id="myTabContent">
                                    ...
                                    <div class="tab-pane fade" id="reviews" role="tabpanel" aria-labelledby="reviews-tab">
                                        <div class="content-parent">
                                            <div class="reviews">
                                                <ul v-if='props.data.reviews.length' class="ul">
                                                    <li v-for='(review, index) in props.data.reviews' class="review-item">
                                                        <div class="review-wrapper">
                                                            <div class="image">
                                                                <img :src="getAvatar(review.user.avatar_path)" alt="">
                                                            </div>
                                                            <div class="content">
                                                                <h2 class="name">{{review.user.name}}</h2>
                                                                <div class="stars">
                                                                    <vue3-star-ratings :disableClick='stardisable' :starSize="'20'" :showControl='false' controlBg='#f05922' v-model="review.rating" />
                                                                </div>
                                                                <p class="text">
                                                                    {{review.detail}}
                                                                </p>
...
const getAvatar = (val) => {
    return avatar_path.value = 'storage/' + val;
};
watchEffect(() => {
...
const requestHandle= ()=>{
            ...
            let bookDate=booking_date.value
            localStorage.setItem(
            "data",
                JSON.stringify({
                        user_business_id: route.params.id,
                        booking_date:bookDate,
                        items: selectedCategories.value.map((item) => {
                        return JSON.stringify({
                        name: item.user_service.name,
                        id: item.id,
                        price: item.charges,
