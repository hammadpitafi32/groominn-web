                    ...
                    <div class="py-5 px-4 rounded-5 bg-light-grey shop-form">
                        <form action="">
                            <div class="form-group mb-3">
                                <label for="name" class="mb-2 fw-bold small">Business Name</label>
                                <MDBInput id="name" v-model="businessName" class="bg-white py-2" />
                            ...
                            <div class="form-group mb-3">
                                ...
                                <MDBTextarea rows="5" v-model="description" id="description" class="bg-white py-2 no-resize" />
                                <span v-if="errors && errors.description" class="text-danger small">{{ errors.description[0] }}</span>
                            </div>
                            <MDBRow>
                                <MDBCol col="12 col-sm-6 col-md-3 ">
                                    ...
                                    <label for="shop-pics" class="custom-drag-label" :class="drag.shop && 'drag-entered'" @dragover="drag.shop = true" @dragleave="drag.shop = false" @dragend="drag.shop = false" @drop="drag.shop = false">
                                        <svg width="26" height="17" viewBox="0 0 26 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M25.8508 10.0979L22.5816 15.5943C22.3272 16.0221 21.963 16.3769 21.5251 16.6236C21.0873 16.8702 20.5912 17 20.0863 17H2.03238C1.19627 17 0.675323 16.1105 1.0966 15.4021L4.36574 9.90569C4.62016 9.47794 4.98443 9.12306 5.42227 8.87642C5.8601 8.62978 6.3562 8.5 6.86111 8.5H24.915C25.7512 8.5 26.2721 9.38953 25.8508 10.0979ZM6.86111 7.08333H21.6667V4.95833C21.6667 3.78471 20.6966 2.83333 19.5 2.83333H12.2778L9.38889 0H2.16667C0.970035 0 0 0.95138 0 2.125V14.4343L3.11806 9.19186C3.89161 7.89128 5.32589 7.08333 6.86111 7.08333Z" fill="#F05922" />
                                        </svg>
                                        <span class="ms-2">Drag & drop your file here</span>
                                        <MDBFile id="shop-pics" @change="uploadPics($event, 'shop')" class="mb-2 custom-file-type" />
                                    </label>
                                    <div class="row mt-3" v-if="shopPics.length > 0">
                                        <div class="col-4 pb-3" v-for="(image, index) in shopPics" :key="index">
                                            <div class="image-wrap">
                                                <img :src="image" alt="" class="img-fluid rounded-5" />
                                                <span class="remove-img" @click="
                                                    ...
                                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        ...
                                                        <path d="M13.3563 11.0018L11.7963 8.55378L10.2962 11.0018H9.15625L11.2803 7.73778L9.15625 4.42578H10.3923L11.9523 6.86178L13.4402 4.42578H14.5803L12.4683 7.67778L14.5923 11.0018H13.3563Z" fill="#F05922" />
                                                        <defs>
                                ...
                                <MDBCol col="12 col-sm-6 col-md-3 sec-col">
                                    ...
                                    <label for="cnic-front" class="custom-drag-label" :class="drag.front && 'drag-entered'" @dragover="drag.front = true" @dragleave="drag.front = false" @dragend="drag.front = false" @drop="drag.front = false">
                                        ...
                                        <span class="ms-2">Drag & drop your file here</span>
                                        <MDBFile id="cnic-front" @change="
                                                uploadPics($event, 'cnic-front')
                                            " class="custom-file-type" accept="image/*" />
                                    </label>
                                    <span v-if="errors && errors.cnic_front" class="text-danger small">{{ errors.cnic_front[0] }}</span>
                                ...
                                <MDBCol col="12 col-sm-6 col-md-3 sec-col">
                                    ...
                                    <label for="cnic-back" class="custom-drag-label" :class="drag.back && 'drag-entered'" @dragover="drag.back = true" @dragleave="drag.back = false" @dragend="drag.back = false" @drop="drag.back = false">
                                        ...
                                        <span class="ms-2">Drag & drop your file here</span>
                                        <MDBFile id="cnic-back" @change="
                                                uploadPics($event, 'cnic-back')
                                            " class="mb-2 custom-file-type" />
                                    </label>
                                    <span v-if="errors && errors.cnic_back" class="text-danger small">{{ errors.cnic_back[0] }}</span>
                                ...
                                <MDBCol col="12 col-sm-6 col-md-3 sec-col">
                                    ...
                                    <label for="shop-liscence" class="custom-drag-label" :class="drag.liscence && 'drag-entered'" @dragover="drag.liscence = true" @dragleave="drag.liscence = false" @dragend="drag.liscence = false" @drop="drag.liscence = false">
                                        ...
                                        <span class="ms-2">Drag & drop your file here</span>
                                        <MDBFile id="shop-liscence" @change="
                                                uploadPics($event, 'liscence')
                                            " class="mb-2 custom-file-type" />
                                    </label>
                                    <span v-if="errors && errors.license" class="text-danger small">{{ errors.license[0] }}</span>
                            ...
                            <MDBRow class="mt-5">
                                <MDBCol col="12" class="text-end pt-4">
                                    <MDBBtn v-if="editMode" @click="router.push('/my-shop')" class="shadow-4-strong">
                                        Cancel
                                    </MDBBtn>
                                    <MDBBtn class="text-white bg-orange shadow-0 text-capitalize px-5 rounded-4 ms-3" :disabled="loading" @click="addShopHandler()">
                                        <span v-if="!loading">{{
                                            editMode
                                            ? "Save Changes"
                                            : "Add shop"
